...
from util.game import daily_puzzle_id, generate_info_embed, generate_puzzle_embed, process_message_as_guess, random_puzzle_id, TicTacToe
from util.messages import DeleteMessageSlash, DeleteMessage
...
class Games(commands.Cog, description="Play some games."):
    ...
    async def slash_info_normal(self, ctx):
        await ctx.send(embed=generate_info_embed(), view=DeleteMessage(ctx))
