...
from util.databases.invite import InviteDB
...
class InviteTracker(commands.Cog, name="Tracker for the bot."):
    ...
    async def on_ready(self):
        ...
        self.invite_db = InviteDB(self.bot)
    ...
    async def on_member_join(self, member: nextcord.Member):
            ...
            data = await self.invite_db.invite_data_existence_check(member.guild.id, inviter.id)
            ...
            await self.invite_db.create_data(member.guild.id, inviter.id, data)
            ...
            if member.guild.icon is not None:
             guild_icon_url = member.guild.icon.url
            else:
                guild_icon_url = "https://discord.com/assets/c09a43a372ba81e3018c3151d4ed4773.png"
    ...
    async def set_invite_log_channel(self, ctx: commands.Context, channel: nextcord.TextChannel):
       ...
       data = await self.invite_db.invite_data_existence_check_by_guild_id(ctx.message.guild.id)
       
       if data:
          data["log_channel"] = channel.id
       
          await self.invite_db.create_data_by_guild_id(ctx.message.guild.id, data)
